...
def shuffle_caption(caption):
    ...
    # if n noun > 2 and n adj > 2 shuffle all (find two adj and one noun not linked to both adj and randomly choose another)
    try:
        if (len(adjectives) > 2 and len(nouns) > 2):
            ...
            adj1 = random.choice([adj for adj in adjectives if (adj, nouns[0]) not in adj_noun and (adj, nouns[1]) not in adj_noun])
            ...
            noun1 = random.choice([n for n in nouns if (adjectives[0], n) not in adj_noun and (adjectives[1], n) not in adj_noun])
            nouns.remove(noun1)
            noun2 = random.choice(nouns)
    ...
    others = [token.text for token in doc if token.pos_ != "NOUN" and token.pos_ != "ADJ" and token.pos_ !="DET" and token.pos_ !="SPACE"]
