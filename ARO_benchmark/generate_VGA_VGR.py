...
ATTRIBUTES = set() # to control which pairs of attribute we add to the dataset
...
def generate_dataset(set_type):
    ...
    for id_image, image_data in tqdm(data_raw.items()):
        ...
        if path.exists(f"GQA_raw/images/{id_image}.jpg"):
            obj_pairs = dict()
            # selection of candidates objects (good size, no duplicates)
            ...
            for (obj1, obj2), relation in obj_pairs.items():
                ...
                if name_obj1 != name_obj2:  # we keep only the objectsof different types
                    ...
                    cpt += 1  # for the name of th image
                    ...
                    # creation of the attributes dataset (if attrributes are different, no duplicates)
                    att_pairs = []
